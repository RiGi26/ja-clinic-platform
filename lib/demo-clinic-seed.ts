...
export async function seedDemoData(db: SupabaseClient, clinicId: string) {
  ...
  // Every doctor/appointment carries a branch (location_id). Ensure the demo
  // clinic has a default branch so fresh seeds stay valid under the NOT NULL
  // contract (B1d).
  let { data: loc } = await db.from('locations').select('id')
    .eq('clinic_id', clinicId).order('created_at', { ascending: true }).limit(1).maybeSingle()
  if (!loc) {
    const { data: newLoc } = await db.from('locations').insert({
      clinic_id: clinicId, name: 'Cabang Utama',
      address: 'Jl. Sudirman No. 1, Jakarta Pusat', phone: '021-12345678',
    }).select('id').single()
    loc = newLoc
  }
  ...
  if (doctors.length === 0) {
    const { data: d } = await db.from('doctors').insert([
      { clinic_id: clinicId, full_name: 'Dr. Sarah Putri',  specialty: 'Dokter Umum',             consultation_fee: 150000, is_active: true, location_id: locationId },
      { clinic_id: clinicId, full_name: 'Dr. Budi Santoso', specialty: 'Spesialis Anak',           consultation_fee: 250000, is_active: true, location_id: locationId },
      { clinic_id: clinicId, full_name: 'Dr. Ahmad Fauzi',  specialty: 'Spesialis Penyakit Dalam', consultation_fee: 300000, is_active: true, location_id: locationId },
    ]).select('id')
    doctors = d ?? []
  }
  ...
  if (!existApts || existApts.length === 0) {
    ...
    await db.from('appointments').insert([
      { clinic_id: clinicId, patient_id: patients[0].id, doctor_id: doctors[0].id, location_id: locationId, scheduled_at: dt(9,0),  complaint: 'Demam dan batuk',       status: 'menunggu',  type: 'booking', queue_number: 1 },
      { clinic_id: clinicId, patient_id: patients[1].id, doctor_id: doctors[0].id, location_id: locationId, scheduled_at: dt(9,30), complaint: 'Sakit kepala',           status: 'dipanggil', type: 'booking', queue_number: 2 },
      { clinic_id: clinicId, patient_id: patients[2].id, doctor_id: doctors[0].id, location_id: locationId, scheduled_at: dt(10,0), complaint: 'Kontrol tekanan darah',  status: 'diperiksa', type: 'walkin',  queue_number: 3 },
      { clinic_id: clinicId, patient_id: patients[3].id, doctor_id: doctors[0].id, location_id: locationId, scheduled_at: dt(10,30),complaint: 'Nyeri sendi',            status: 'selesai',   type: 'booking', queue_number: 4 },
    ])
  }
