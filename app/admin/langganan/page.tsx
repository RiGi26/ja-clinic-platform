...
// ============================================================
// /admin/langganan — subscription page (distinct from /admin/billing, which is
// PATIENT invoices). Shows the clinic's current plan/status and the Clinic plans,
// with a single CTA that mints a checkout token (/api/billing/upgrade) and hands
// off to the superadmin payment page. Core DB is SoR; prices come from its public
// /api/public/plans feed.
//
...
const ERROR_NOTE: Record<string, string> = {
  forbidden: 'Hanya admin klinik yang dapat mengelola langganan.',
...
async function fetchClinicPlans(): Promise<PublicPlan[]> {
  try {
...
export default async function LanggananPage({
  ...
  const [ent, plans, sp] = await Promise.all([
    getClinicEntitlements(user.clinic_id),
    fetchClinicPlans(),
    searchParams,
  ])
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <header className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-500">Kelola paket Webzoka Clinic Anda.</p>
      </header>
