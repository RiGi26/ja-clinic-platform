...
export default async function AdminDashboardPage() {
  ...
  const [
    ...
    db.from('appointments')
      .select(`
        id, scheduled_at, status, complaint,
        patients(no_rm, users(full_name)),
      ...
      .limit(8),
    db.from('billing').select('total').eq('status', 'paid').gte('created_at', monthStart),
    db.from('doctors').select('*', { count: 'exact', head: true }).eq('is_active', true),
  ])

  const pendapatan = (monthBilling ?? []).reduce((s, b) => s + (b.total ?? 0), 0)

  const stats = [
    { label: 'Total Pasien',           value: (totalPatients ?? 0).toLocaleString('id-ID'), Icon: Users,        change: '' },
    { label: 'Appointment Hari Ini',   value: String(todayAppointments ?? 0),               Icon: Calendar,     change: '' },
    { label: 'Pendapatan Bulan Ini',   value: fmtRupiah(pendapatan),                        Icon: DollarSign,   change: '' },
    { label: 'Antrian Sekarang',       value: String(waitingCount ?? 0),                    Icon: Clock,        change: '' },
  ]
    ...
    <div className="space-y-7">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {(!todayApts || todayApts.length === 0) ? (
            ...
            <div className="space-y-3">
              ...
              {(todayApts as any[]).map((apt, i) => {
                const patientName = apt.patients?.users?.full_name ?? 'Pasien'
                const doctorName  = apt.doctors?.users?.full_name  ?? 'Dokter'
                ...
                return (
                  <div
                    key={apt.id}
                    className="bg-white rounded-2xl p-5 border border-gray-100 hover:shadow-md transition-shadow animate-fade-in-up flex items-center gap-4"
                    style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.04)', animationDelay: `${i * 40}ms` }}
                  >
                    <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${avatarGradient(patientName)} flex items-center justify-center text-white font-black text-base flex-shrink-0`}>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-[20px] border border-gray-100 p-6"
            ...
            <div className="flex mt-3">
              {Array.from({ length: Math.min(activeDoctors ?? 0, 6) }).map((_, i) => (
                <div
                  key={i}
                  className="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-400 to-blue-600 border-2 border-white flex-shrink-0"
          ...
          <div className="bg-white rounded-[20px] border border-gray-100 p-6"
            style={{ boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
            <p className="text-[11px] uppercase tracking-widest font-bold text-gray-400 mb-2">Pendaftaran Baru</p>
            <p className="text-4xl font-black text-[#0C2340] tabular-nums">{totalPatients ?? 0}</p>
            <div className="flex items-center gap-1.5 mt-2">
              <TrendingUp size={13} className="text-emerald-500" />
              <p className="text-xs text-emerald-600 font-bold">Total pasien terdaftar</p>
            </div>
          </div>
          ...
          <Link
            ...
            className="w-full flex items-center justify-center gap-2 py-3.5 bg-[#0891B2] hover:bg-[#0e7490] text-white font-black rounded-2xl transition-all text-sm shadow-sm"
          >
